...
**For comprehensive or technical diagrams, you MUST build the JSON one section at a time.** Do NOT attempt to generate the entire file in a single pass. Large diagrams produce thousands of lines of JSON, and generating everything at once leads to worse quality — missed bindings, inconsistent spacing, and harder-to-debug errors. Section-by-section is better in every way.
...
- **Don't generate the entire diagram in one response.** Large diagrams risk truncated or broken JSON. Even if it fits, splitting into sections produces better results.
- **Don't delegate to a subagent** to generate the JSON. The subagent won't have this skill's instructions in context, and the coordination overhead negates any benefit.
...
```bash
cd .github/skills/excalidraw-diagram-skill/scripts && uv run python render_excalidraw.py <path-to-file.excalidraw>
```
