          ...
          {sortedTasks.length === 0 ? (
            ...
            >
              ...
              <span className="text-xs text-text-secondary">
                {tasks.length > 0 ? "No tasks match your filters" : "No issues discovered yet"}
              </span>
              <span className="text-[10px] text-text-muted/50 text-center max-w-[260px]">
                {tasks.length > 0
                  ? "Try adjusting the severity, status, or domain filters above"
                  : isRunning
                    ? "Agents are actively exploring your app — tasks will appear here as issues are discovered, diagnosed, and fixed"
                    : "Start the pipeline to let agents explore your app. They will create tasks as they discover bugs, UX issues, and improvement opportunities."}
              </span>
            </motion.div>
          ) : (
